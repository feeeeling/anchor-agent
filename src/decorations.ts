  ...
  refresh(): void {
    for (const editor of vscode.window.visibleTextEditors) {
      ...
      for (const task of this.tasks.list()) {
        if (task.documentUri !== editor.document.uri.toString() || HIDDEN_STATES.has(task.taskState)) {
          continue;
        }
        ...
        if (task.taskState === "conflicted" || task.anchorState === "modified" || task.anchorState === "orphaned") {
